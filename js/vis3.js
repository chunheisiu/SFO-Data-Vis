(function() {
  ...
  d3.csv("data.csv", type, function(error, data) {
    ...
    var airbus = geo.filter(function(d) { return d.key == "Airbus"; })
    ...
    airbus = flatten(airbus).sort(function(a,b) { return a.key.localeCompare(b.key); });
    //console.log(airbus);
    ...
    planes.append("path")
      ...
      .attr("transform", function(d, i) {
        var j = i > 4 ? i - 5 : i;
        return "translate(" + (-50 + j * 200) + "," + (margin.top + 50 + (Math.floor(i / 5) * 250)) + ") scale(0.015, -0.015)";
      })
      .attr("fill", function(d, i) { return "url(#airbus-grad" + i + ")"; })
      .on("mouseover", function(d, i) {
        d3.select(this).classed("plane-active", true);
        d3.select("#airbus-planeDesc" + i).style("opacity", "1");
      })
      .on("mouseout", function(d, i) {
        d3.select(this).classed("plane-active", false);
        d3.select("#airbus-planeDesc" + i).style("opacity", "0");
      });
    ...
    planes.append("text")
      ...
      .attr("transform", function(d, i) {
        var j = i > 4 ? i - 5 : i;
        return "translate(" + (j * 200 + 30) + "," + (margin.top + (Math.floor(i / 5) * 250)) + ")";
      })
      .text(function(d) { return d.key; });

    var planeDesc = planes.append("g")
      .attr("id", function(d, i) { return "airbus-planeDesc" + i; })
      .attr("transform", function(d, i) {
        var j = i > 4 ? i - 5 : i;
        return "translate(" + (j * 200 + 30) + "," + (margin.top + (Math.floor(i / 5) * 250)) + ")";
      })
    ...
    planeDesc.append("text")
      .attr("id", function(d, i) { return "airbus-planeDesc-dom" + i; })
      .attr("class", "text-dom")
      .attr("font-family", "sans-serif")
      .attr("font-size", 12)
      .attr("text-anchor", "middle")
    ...
    planeDesc.append("text")
      .attr("id", function(d, i) { return "airbus-planeDesc-intl" + i; })
